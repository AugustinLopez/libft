...
size_t	strncmp_speed_test(int len, int second, int (str)(const char *, const char *, size_t))
{
	clock_t			chrono = 0;
	clock_t			start = 0;
	size_t			i = 0;
...
int	main(int ac, char **av)
{
	...
	size = atoi(av[1]);
	printf("%-7d: ", size);
	n = strncmp_speed_test(size, 5, *ft_strncmp);
	d = strncmp_speed_test(size, 5, *strncmp);
	printf("%.f%%\n", (n + 0.0) / (d + 0.0) * 100);
	return (0);
}
